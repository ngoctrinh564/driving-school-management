...
namespace driving_school_management.Services
{
    public class PaymentInvoiceService
    {
        ...
        public PaymentInvoiceService(IConfiguration configuration)
        {
            _connectionString = configuration.GetConnectionString("OracleDb");
        }

        public PaymentInvoiceDto? GetInvoiceDetail(int userId, int phieuId)
        {
            PaymentInvoiceDto? result = null;

            using (var conn = new OracleConnection(_connectionString))
            using (var cmd = new OracleCommand("SP_PAYMENT_INVOICE_DETAIL", conn))
            {
