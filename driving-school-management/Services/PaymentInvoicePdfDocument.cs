...
{
    public class PaymentInvoicePdfDocument : IDocument
    {
        private readonly PaymentInvoiceDto _model;
        private readonly DateTime _exportedAt;
        private readonly string _brandLogoPath;
        private readonly string? _paymentLogoPath;

        public PaymentInvoicePdfDocument(
            PaymentInvoiceDto model,
            DateTime exportedAt,
        ...
        private void ComposeContent(IContainer container)
        {
            container.PaddingTop(18).Column(col =>
            {
                ...
                col.Item().Text("THÔNG TIN KHÓA HỌC")
                    .FontSize(11).Bold().FontColor("#64748B");

                col.Item().Element(x => ComposeInfoGrid(x, new List<(string Label, string Value)>
                {
                    ("Khóa học", _model.TenKhoaHoc),
                    ("Mã khóa học", _model.KhoaHocId.ToString()),
                    ("Hạng", _model.TenHang),
                    ("Loại phương tiện", _model.LoaiPhuongTien),
                    ("Địa điểm", _model.DiaDiem),
                    ("Ngày bắt đầu", _model.NgayBatDau?.ToString("dd/MM/yyyy HH:mm:ss") ?? ""),
                    ("Ngày kết thúc", _model.NgayKetThuc?.ToString("dd/MM/yyyy HH:mm:ss") ?? ""),
                    ("Trạng thái khóa học", _model.TrangThaiKhoaHoc),
        ...
        private void ComposeHeroBlock(IContainer container)
        {
            container
                ...
                .Column(col =>
                {
                    ...
                    col.Item().Text(_model.TenKhoaHoc)
                        .FontSize(21).Bold().FontColor("#0F172A");

                    col.Item().Row(row =>
                    {
                        row.AutoItem().Element(x => ComposeMiniTag(x, _model.TenHang));
                        row.AutoItem().PaddingLeft(8).Element(x => ComposeMiniTag(x, _model.LoaiPhuongTien));
                        row.AutoItem().PaddingLeft(8).Element(x => ComposeMiniTag(x, _model.TrangThaiKhoaHoc));
                    });
